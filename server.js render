...
if (!NIM_API_KEY) {
  ...
  console.error('Please add it in Render dashboard: Environment → Add Environment Variable');
}
  ...
  try {
    ...
    if (!NIM_API_KEY) {
      return res.status(500).json({
        error: {
          message: 'NIM_API_KEY not configured. Please add it in Render dashboard.',
    ...
    if (!nimModel) {
      // Try to use the model directly if it's available
      try {
      ...
      // If still no model, use intelligent fallback
      if (!nimModel) {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`🔑 API Key configured: ${NIM_API_KEY ? 'YES' : 'NO (⚠️  REQUIRED!)'}`);
});
